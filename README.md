...
- 绩点规则可查看（管理员可编辑，存储在 `data/gpa_rules.txt`）
- 支持批量重算全部学生绩点
...
### 权限管理
- 管理员（admin）：全部功能，含用户管理、操作日志、数据备份恢复
- 老师（数字工号）：学生搜索、课程管理、成绩录入/批量导入/批量调整（仅限自己创建的课程）、修改/删除（仅自己录入的）

### 数据校验
- 分数 0-100 范围校验
- 学号必须 7 位数字，课程代码仅允许数字和 `.`
- 老师账号必须 7 位数字，密码 8-12 位
- 学号、课程代码、账号唯一性校验，重复时中文提示
...
> 默认管理员仅在首次启动时创建，之后启动不会重置密码；首次登录会要求修改密码。
